...
;Source : https://gist.github.com/anonymous1184/b251cd8407a379d4965791585887cfce#file-appvol-ahk

AppVol(Target := "A", Level := 0) {
    ; --- Normalisation du paramètre Target ---
    if (Target ~= "^[-+]?\d+$") {
        Level := Target
        Target := "A"
    ...
    ; --- Récupération du nom du process cible ---
    try {
    ...
    appNameNoExt := StrLower(StrReplace(appName, ".exe"))
    ...
    ; --- Parcours des sessions audio ---
    loop cSessions {
        ComCall(4, IAudioSessionEnumerator, "Int", A_Index - 1, "Ptr*", &IAudioSessionControl := 0)
        IAudioSessionControl2 := ComObjQuery(IAudioSessionControl, "{BFB7FF88-7239-4FC9-8FA2-07C950BE9C6D}")
        ObjRelease(IAudioSessionControl)

        ; PID du process audio
        ComCall(14, IAudioSessionControl2, "UInt*", &pid := 0)

        ; --- Correction 1 : ignorer les sessions système ---
        if (pid = 0) {
            continue
        }

        ; --- Correction 2 : ProcessGetName peut échouer ---
        try procName := StrLower(ProcessGetName(pid))
        catch {
            continue
        }

        ; --- Correction 3 : Chrome/Edge multi-process ---
        if (procName != appName && procName != appNameNoExt) {
            continue
        }
